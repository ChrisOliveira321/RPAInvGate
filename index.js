...
// ✅ novo
const { saveTicketDetails } = require('./src/db/ticketRepo');
...
;(async () => {
  ...
  const cards = await myWork.readCards(); // ✅ aqui já salva triagem no DB
  ...
  for (const t of ticketsToOpen) {
    ...
    try {
      ...
      console.log('⚠️ Falha ao salvar DETALHE no DB:', e.message);
    }
